...
"\x3d"+encodeURIComponent(config[prop]))});return""+baseURI+(params.length?"?"+params.join("\x26"):"")}function setupLink(node,newEl,config){var baseURI=checkIfCallback(config.protocolIf,config)?WHATSAPP_API_PROTOCOL:WHATSAPP_API_URL;var options={text:node.dataset.message,phone:node.dataset.whatsapp};if(newEl.tagName==="A")newEl.href=buildLink(options,baseURI);if(typeof config.elementCallback==="function")config.elementCallback(node,function(params){if(typeof params==="string")params={text:params};
